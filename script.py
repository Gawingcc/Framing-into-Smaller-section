...
def get_beam_weight(beam):
    """Extracts weight from beam type name (e.g., 'W18X35' -> 35)"""
    b_type = doc.GetElement(beam.GetTypeId())
    type_name = b_type.get_Parameter(BuiltInParameter.SYMBOL_NAME_PARAM).AsString() or ""
    # Extract weight from names like "W18X35", "W24X68", etc.
    if 'X' in type_name:
        try:
            return float(type_name.split('X')[-1])
        except:
            return 0
    return 0

def is_column(element):
    """Check if element is a column based on category"""
    try:
        cat = element.Category
        return cat.Name == "Structural Columns"
    except:
        return False
...
for beam in all_beams:
    ...
    for i in [0, 1]:
        ...
        for nb in neighbors:
            ...
            # Skip if neighbor is a column
            if is_column(nb):
                continue
            nb_weight = get_beam_weight(nb)
            # Only flag if supporting beam is lighter (smaller weight number)
            if nb_weight > 0 and nb_weight < weight:
                is_problematic = True
                nb_type = doc.GetElement(nb.GetTypeId()).get_Parameter(BuiltInParameter.SYMBOL_NAME_PARAM).AsString()
                supporting_info = nb_type
                break
    ...
    if is_problematic:
        ...
        report_data.append("{},{},{},{}".format(beam.Id.IntegerValue, mark, type_name, supporting_info))
...
try:
    ...
    with open(report_path, 'w') as f:
        f.write("Beam ID,Mark,Type Name,Supporting Beam Type\n")
        f.write("\n".join(report_data))
